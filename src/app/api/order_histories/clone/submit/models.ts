...
export const SubmitCloneOrderSchema = z.object({
  clonedFromOrderHistoryId: z.string().uuid(),
  systemFields: z.object({
    ...
    paxMajor: z.number().int("Must be a whole number"), //todo: not sure if accepted
    paxMinor: z.number().int("Must be a whole number"), //todo: not sure if accepted
  }),
...
export type SubmitCloneOrderRequest = z.infer<typeof SubmitCloneOrderSchema>;
